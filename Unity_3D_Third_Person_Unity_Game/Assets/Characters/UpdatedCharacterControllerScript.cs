...
public class UpdatedCharacterControllerScript : MonoBehaviour
{
    ...
    public int includeAllButPoles = LayerMaskCollection.includeAllButPoles;
    ...
    public Vector2 swimmingStrokeForwardInitialSpeed = new Vector2(0.15f, 0.06f);
    public float swimmingStrokeUpwardInitialSpeed = 0.2f;
    public float swimmingGravityIncrement = -0.003f;
    ...
    public int attackingTimer = 30;
    public int attackingTime = 30;
    ...
    float jump;
    float enableHold;
    float enableClick; // both values use the same input but enableHold is used just for seeing if the button is held down
    //float groundPound;
    //float speedUp;
    //float strokeForward;
    float attack;
    ...
    public int groundPoundTimeLimit = 30; // frames
    ...
    public float poleDescendRate = -0.2f;
    public float poleRotateSpeed = 2f;
    ...
    public float explosionKnockBackDecay = 0.92f;
    ...
    void Start()
    {
        //Debug.Log(Mathf.SmoothDamp(0, 0.15f, ref speedSmoothVelocity, 0.5f));
        ...
        verticalRaycastOffset = maximumPlayerHorizontalVelocity * Mathf.Tan(SlipperyFloorWallArc) + 0.0f; // should add a little extra like 0.01f
    ...
    void CollectAndConfigureInputs()
    {
        ...
        {
            // inputs are allowed
            input = keyBindScript.VHinput;
            enableHold = keyBindScript.enable.currentValue;
            //Debug.Log(enableHold);
            // buttons
            jump = keyBindScript.jump.currentValue;
            if (jump == 0)
            {
                jumpEnable = true;
            }
            ...
            enableClick = enableHold;
            if (enableClick == 0)
            {
                enableClickEnable = true;
            }
            else
            {
                if (enableClickEnable)
                {
                    enableClickEnable = false;
                }
                else
                {
                    enableClick = 0;
            ...
            attack = keyBindScript.attack.currentValue;
            if (attack == 0)
            {
                attackEnable = true;
            }
    ...
    void ExecuteState()
    {
        ...
        else if (characterState == CharacterState.ATTACKING_ONE)
        {
            PlayerAttackingOneState();
        }
    }
    ...
    void ExecuteNoDirectionRaycasts()
    {
        ...
        if (!wasGrounded || XZ == Vector3.zero)
        {
            ...
            if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightBottom, 0) + XZNormalizedForward * (horizontalRaycastOffset - forwardThickness), -XZNormalizedForward, out noDirection, transX, includeAllButPoles))
            {
                WallCollisionMoveBack(noDirection);
            }
            if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightBottom, 0) - XZNormalizedRight * (horizontalRaycastOffset - forwardThickness), XZNormalizedRight, out noDirection, transX, includeAllButPoles))
            {
                WallCollisionMoveBack(noDirection);
            }
            if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightBottom, 0) + XZNormalizedRight * (horizontalRaycastOffset - forwardThickness), -XZNormalizedRight, out noDirection, transX, includeAllButPoles))
    ...
    void ExecuteMovementForwardRaycasts()
    {
        ...
        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightBottom, 0) - transform.forward * (horizontalRaycastOffset - forwardThickness), XZNormalizedForward, out hitForwardBottom, transXM, includeAllButPoles))
        {
            WallCollisionSlide(hitForwardBottom);
        }

        RaycastHit hitForwardTop;
        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightTop, 0) - transform.forward * (horizontalRaycastOffset - forwardThickness), XZNormalizedForward, out hitForwardTop, transXM, includeAllButPoles))
        {
            WallCollisionSlide(hitForwardTop);
        }

        RaycastHit hitForwardMiddle1;
        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightMiddle1, 0) - transform.forward * (horizontalRaycastOffset - forwardThickness), XZNormalizedForward, out hitForwardMiddle1, transXM, includeAllButPoles))
        {
            WallCollisionSlide(hitForwardMiddle1);
        }

        RaycastHit hitForwardMiddle2;
        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightMiddle2, 0) - transform.forward * (horizontalRaycastOffset - forwardThickness), XZNormalizedForward, out hitForwardMiddle2, transXM, includeAllButPoles))
        {
            WallCollisionSlide(hitForwardMiddle2);
        }

        RaycastHit hitForwardMiddle3;
        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightMiddle3, 0) - transform.forward * (horizontalRaycastOffset - forwardThickness), XZNormalizedForward, out hitForwardMiddle3, transXM, includeAllButPoles))
        {
    ...
    void ExecuteMovementUpwardRaycasts()
    {
        ...
        if (Physics.Raycast(transform.position + new Vector3(0, upwardThickness - verticalCeilingRaycastOffset, 0), transform.up, out hitUpward, transYUp, includeAllButPoles))
        {
    ...
    void ExecuteMovementDownwardRaycasts()
    {
        ...
        if (Physics.Raycast(transform.position + new Vector3(0, (verticalRaycastOffset - downwardThickness) + 0.01f, 0), -transform.up, out hitDownward, distanceDownward, includeAllButPoles))
        {
    ...
    void WallCollisionSlide(RaycastHit hit)
    {
        if (hit.normal.y >= wallLowerLimitNormalY && hit.normal.y <= wallUpperLimitNormalY)
        {
            ...
            RaycastHit pinch;
            if (slideApplied || Physics.Raycast(hit.point, hit.normal, out pinch, forwardThickness * 2 + 0.01f, includeAllButPoles) || angle > 135)
            {
                ...
                xzModified = true;
                if (!isGrounded && currentSpeed > runSpeed/2)
                {
    ...
    void FloorCollision(RaycastHit hit)
    {
        if (hit.normal.y > slipperyFloorLowerLimitNormalY) // the boundary between wall and slippery floor
        {
            ...
            if (Physics.Raycast(transform.position, hitNormalXZ, out ray, 1.0f, includeAllButPoles))
            {
                useRegFloor = true;
            }
    ...
    void EdgeCollision()
    {
        if (universalMovementVector.y <= 0 && !isGrounded)
        {
            if (input.magnitude > 0.05f)
            {
                ...
                if (Physics.Raycast(transform.position + transform.forward * (1 + forwardThickness) + new Vector3(0, forwardHeightTop - maxFallVelocity, 0), -transform.up, out hitEdgeDownward, -maxFallVelocity, includeAllButPoles))
                {
                    if (hitEdgeDownward.normal.y >= 1 - 0.09f)
                    {
                        if (hitEdgeDownward.transform.tag != "notEdge")
                        {
                            if (Physics.Raycast(new Vector3(transform.position.x, hitEdgeDownward.point.y - 0.1f, transform.position.z), transform.forward, out hitEdgeForward, forwardThickness + 0.5f, includeAllButPoles))
                            {
    ...
    void RunningState()
    {
        ...
        else
        {
            ...
            float targetSpeed = runSpeed * input.magnitude * floorNormal.y;
            ...
            if(currentSpeed < 0)
            {
                currentSpeed = 0; // sometimes smooth damp returns a small negative value
            }
            if (noFriction)
            {
            ...
            if (jump > 0)
            {
                ...
                if (enableHold > 0)
                {
                    if (input.magnitude > 0.51f && !touchingWall)
                    {
    ...
    void FallingState()
    {
        if (isGrounded)
        {
            ...
            if (fallDamageTimer <= 0)
            {
                ...
                if (playerStats.UpdatePlayerHealth_IsDead(1))
                {
        ...
        if (hangStateTimer <= 0)
        {
            ...
            if (input != Vector2.zero)
            {
                ...
                if (tempInput.y >= 0.5f)
                {
                    ...
                    if (!Physics.Raycast(transform.position + new Vector3(0, forwardHeightTop + 0.5f, 0), transform.forward, out allowedToClimb, 2.0f, includeAllButPoles))
                    {
                ...
                else if (tempInput.x >= 0.5f)
                {
                    // move right
                    // first check if more edge is available
                    RaycastHit moveRight;
                    if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightTop - 0.1f, 0) + transform.right * 1f, transform.forward, out moveRight, 0.7f, includeAllButPoles))
                    {
                        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightTop + 0.1f, 0) + transform.right * 1f + transform.forward * 1f, -transform.up, out moveRight, 0.2f, includeAllButPoles))
                        {
                            // more edge
                ...
                else if (tempInput.x <= -0.5f)
                {
                    // move left
                    RaycastHit moveLeft;
                    if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightTop - 0.1f, 0) - transform.right * 1f, transform.forward, out moveLeft, 0.7f, includeAllButPoles))
                    {
                        if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightTop + 0.1f, 0) - transform.right * 1f + transform.forward * 1f, -transform.up, out moveLeft, 0.2f, includeAllButPoles))
                        {
                            // more edge
                            transform.Translate(-transform.right * 0.1f, Space.World);
                        }
        ...
        if (input != Vector2.zero)
        {
            ...
            else if (input.y <= -0.25f)
            {
                ...
                else
                {
                    ...
                    RaycastHit ground;
                    if (Physics.Raycast(transform.position - transform.forward * 0.5f, -transform.up, out ground, -poleDescendRate, includeAllButPoles))
                    {
                        // ground detected
    ...
    void PlayerAttackingOneState()
    {
        ...
        {
            if (!foundTarget && attackingTimer < attackingTime/2)
            {
                RaycastHit attack;
                if (Physics.Raycast(transform.position + new Vector3(0, forwardHeightMiddle1, 0), transform.forward, out attack, attackDistance, includeAllButPoles))
                {
                    // we hit something
                    if (attack.transform.tag == "Punchable")
                    {
                        attack.transform.GetComponent<Punched>().PlayerHasPunched();
                    }
                    // after comfirming a punch we stop the raycast for the duration of the attack state
                    foundTarget = true;
                }
            }
            attackingTimer--;
    ...
    void SwimmingStrokeForward()
    {
        ...
        if (isGrounded)
        {
            // we want the player to slightly move up during a forward stroke but since we truncate
            // to the ground if within a certain range we need a little extra help getting up
    ...
    void PlayerAttack()
    {
        // in this function we halt the universal movement vector (maybe allow a small movement forward)
        // then we raycast outward a distance to check for attackable objects (including buttons)
        // set the anim to the attack anim
        // set the character to the attack state (which will still check if grounded)
        // then back to the stationary state
        // later might want to include a running attack where we still run but also punch (hard to animate)
        universalMovementVector = Vector3.zero;
        anim.SetTrigger("AttackOne");
        characterState = CharacterState.ATTACKING_ONE;
    ...
    // these next few functions are for outside access
    ...
    public void InjureCharacter(AttackData data)
    {
